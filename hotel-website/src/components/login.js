...
import { Button, Card, CardBody, CardGroup, Col, 
  ...
  TabPane, Row, TabContent} from 'reactstrap';

import {signIn,loginDataToken} from '../api/auth'
import fakeAuth from '../api/fakeAuth'
//import {loginDataBase} from '../api/room'
  ...
  handleSubmit=(event)=>{   
    ...
    if(tab==='1'){
      signIn(username, password).then(data=>{
        if(data.status ==="true"){
          fakeAuth.authenticate("ADMIN","ADMIN");
